...
IF NOT EXIST "tailwindcss.exe" (
    ...
    SET "Download_Link=https://github.com/tailwindlabs/tailwindcss/releases/download/v%VERSION%/tailwindcss-windows-x64.exe"
    SET "Download_Location=tailwindcss.exe"
    CALL :Any_Downloader
    ECHO.
)

FOR /R %%A in (docs\assets\stylesheets\src\*.css) DO (
    ECHO  -^> %%A
    START /B tailwindcss -i "%%~dpA/%%~nxA" -o "docs\assets\stylesheets\dist\%%~nxA" --watch
)
